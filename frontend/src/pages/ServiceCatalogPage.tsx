...
export default function ServiceCatalogPage() {
  ...
  const [createForm, setCreateForm] = useState({ name: '', description: '', suggestedPrice: '', active: true })
  ...
  async function load() {
    ...
    try {
      ...
      const res = await api.serviceCatalog.list(t, { q: q.trim() || undefined, active, page, pageSize })
      setItems(res.data || [])
  ...
  async function runConfirmedAction() {
    ...
    try {
      if (pendingAction.kind === 'create') {
        await api.serviceCatalog.create(t, {
        ...
        setCreateOpen(false)
        setCreateForm({ name: '', description: '', suggestedPrice: '', active: true })
      }

      if (pendingAction.kind === 'update' && editForm) {
        await api.serviceCatalog.update(t, editForm.id, {
          name: editForm.name,
      ...
      if (pendingAction.kind === 'delete' && pendingAction.targetId) {
        await api.serviceCatalog.remove(t, pendingAction.targetId)
      }
  ...
  return (
    <div>
      <div style={{ marginBottom: 18, display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: 26, fontWeight: 900 }}>Servicios (Catálogo)</div>
          <div style={{ fontSize: 13, opacity: 0.75, marginTop: 4 }}>Gestioná trabajos, precios y estado.</div>
        </div>
        <Button onClick={() => setCreateOpen(true)}><Plus size={14} /> Nuevo servicio</Button>
      </div>
      ...
      <Card style={{ marginBottom: 12 }}>
        <CardSection style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: 2, minWidth: 220 }}>
            ...
            <Input value={q} onChange={(e) => { setQ(e.target.value); setPage(1) }} placeholder='Buscar por nombre o descripción...' style={{ paddingLeft: 34 }} />
          </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: 'left', padding: 12 }}>Descripción</th>
                ...
                <tbody>
                  {filtered.map((it) => (
                    <tr key={it.id} style={{ borderTop: '1px solid rgba(148,163,184,0.2)' }}>
                      ...
                      <td style={{ padding: 12, opacity: 0.85 }}>{it.description || '—'}</td>
      ...
      <Modal open={confirmOpen} title='Confirmar acción' onClose={() => setConfirmOpen(false)}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ fontSize: 14 }}>
            {pendingAction?.kind === 'delete' ? '¿Seguro que querés eliminar este servicio?' : '¿Confirmás guardar los cambios?'}
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
